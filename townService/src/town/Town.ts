...
import { findPlayerMatchHistory } from '../PlayerData/MatchHistory';
...
export default class Town {
  ...
  async addPlayer(userName: string, socket: CoveyTownSocket): Promise<Player> {
    ...
    socket.on('disconnect', () => {
      ...
      // When a player disconnects, automatically quit them from any ongoing card games
      for (const interactable of this._interactables) {
        if (interactable instanceof CardGameArea) {
          for (const ongoingGame of interactable.ongoingGames) {
            if (
              ongoingGame.activePlayers.some(
                n => ongoingGame.players[n].playerId === newPlayer.googleEmail,
              )
            ) {
              interactable.quitOngoingCardGame(ongoingGame.id, newPlayer);
            }
          }
        }
      }
    ...
    // Register an event listener for the client socket: if the client updates their
    // login status, inform the CoveyTownController
    socket.on(
      'playerLoginLogout',
      (googleEmail: string | undefined, googleAccountName: string | undefined) => {
        this._updatePlayerLoginStatus(newPlayer, googleEmail, googleAccountName);
      },
    );
    ...
    // Fetch the match history of a player when the client requests it:
    socket.on('requestMatchHistory', (googleEmail: string) => {
      findPlayerMatchHistory(googleEmail).then(matchHistory => {
        this._broadcastEmitter.emit('matchHistory', [googleEmail, matchHistory]);
      });
    });
  ...
  /**
   * Updates the login status of a player within the town
   *
   * @param player Player to update location for
   * @param googleEmail New Google email for this player
   * @param googleAccountName New Google account name for this player
   */
  private _updatePlayerLoginStatus(
    player: Player,
    ...
    this._broadcastEmitter.emit('playerLoginChanged', player.toPlayerModel());
  }
   ...
   * with the specified ID or if there is no video URL specified
   */
  public addCardGameArea(cardGameArea: CardGameAreaModel): boolean {
    const area = this._interactables.find(
      eachArea => eachArea.id === cardGameArea.id,
    ) as CardGameArea;
    if (!area) {
      return false;
    }
    area.updateModel(cardGameArea);
    area.addPlayersWithinBounds(this._players);
    this._broadcastEmitter.emit('interactableUpdate', area.toModel());
    return true;
  }
  ...
  public initializeFromMap(map: ITiledMap) {
    ...
    const cardGameAreas = objectLayer.objects
      .filter(eachObject => eachObject.type === 'CardGameArea')
      .map(eachPSAreaObj => CardGameArea.fromMapObject(eachPSAreaObj, this._broadcastEmitter));

    this._interactables = this._interactables
      .concat(viewingAreas)
      .concat(conversationAreas)
      .concat(posterSessionAreas)
      .concat(cardGameAreas);
