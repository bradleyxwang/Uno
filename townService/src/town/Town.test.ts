...
import { DeepMockProxy, mockClear, mockDeep, mockReset, mock } from 'jest-mock-extended';
...
describe('Town', () => {
  ...
  describe('Socket event listeners created in addPlayer', () => {
    describe('on socket disconnect', () => {
      ...
      it('Removes the player from any ongoing games in a CardGameArea by invoking the quitOngoingCardGame method', async () => {
        // Load in a map with a card game area
        town.initializeFromMap(testingMaps.twoCard);
        expect(
          town.addCardGameArea({
            id: 'Name1',
            ongoingGames: [],
            nonStartedGames: [],
          }),
        ...
        // Create an ongoing game containing the player who will disconnect
        const dummyPlayer1 = {
          playerId: '123@gmail.com',
          username: 'player 1',
        };
        const dummyPlayer2 = {
        ...
        const googleEmail = '123@gmail.com';
        const googleAccountName = 'First Last';
        const cardGameArea = town.getInteractable('Name1') as CardGameArea;
        const spy = jest.spyOn(CardGameArea.prototype, 'quitOngoingCardGame').mockImplementation();

        // Player who will disconnect has same google email as dummyPlayer1
        player.googleEmail = googleEmail;
        player.googleAccountName = googleAccountName;
        cardGameArea.ongoingGames.push(dummyOngoingGame);
    ...
    describe('playerLoginLogout', () => {
      it('Emits a playerLoginChanged event and updates the players google details', () => {
        const googleEmail = '123@gmail.com';
        const googleAccountName = 'First Last';
        playerTestData.login(googleEmail, googleAccountName);
        const lastEmittedMovement = getLastEmittedEvent(townEmitter, 'playerLoginChanged');
        expect(lastEmittedMovement.id).toEqual(playerTestData.player?.id);
        expect(lastEmittedMovement.googleEmail).toEqual(googleEmail);
        expect(lastEmittedMovement.googleAccountName).toEqual(googleAccountName);
        expect(player.googleEmail).toEqual(googleEmail);
        expect(player.googleAccountName).toEqual(googleAccountName);
      });
    });

    describe('requestMatchHistory', () => {
      it('Emits a matchHistory event and invokes the "findPlayerMatchHistory" method in MatchHistory', async () => {
        const spy = jest
          .spyOn(MatchHistoryFunctions, 'findPlayerMatchHistory')
          .mockImplementation(() => Promise.resolve([]));
        const requestMatchHistoryCallback = getEventListener(
          playerTestData.socket,
          'requestMatchHistory',
        ) as (googleEmail: string) => void;

        await requestMatchHistoryCallback('123@gmail.com');
        const lastEvent = await getLastEmittedEvent(townEmitter, 'matchHistory');
        expect(lastEvent).toEqual(['123@gmail.com', []]);
        expect(spy).toBeCalled();
      });
    });
  ...
  describe('[OMG4 initializeFromMap]', () => {
    ...
    it('Creates a CardGameArea instance for each region on the map', async () => {
      town.initializeFromMap(testingMaps.twoCard);
      const cardGameArea1 = town.getInteractable('Name1');
      const cardGameArea2 = town.getInteractable('Name2');
      expect(cardGameArea1.id).toEqual('Name1');
      expect(cardGameArea1.boundingBox).toEqual({ x: 40, y: 120, height: 237, width: 326 });
      expect(cardGameArea2.id).toEqual('Name2');
      expect(cardGameArea2.boundingBox).toEqual({ x: 612, y: 120, height: 266, width: 467 });
      expect(town.interactables.length).toBe(2);
    });
